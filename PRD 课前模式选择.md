...
| 负责 Owner | Alexis Lin |
| 关联 | `Demo UX 交互原型.html`（可交互原型 · 手机 + ATOM 双端 · 预览优先）· `Code Flutter/`（代码实现）· `Demo UX 素材图/`（`UI-spec-en/zh.png` 全 24 屏素材 + `flow-board.png` 流程图）· `Tips 识别准确度指南.md` · `README.md` |
...
| 选择模式 → **门槛弹窗**（锁定 AI 模式） | 点已锁定卡片 | 点背景 / 「以后再说」 | 缺设备 / 缺 Plus / 额度用尽三种 |
| 选择模式 → **课前须知**（Get set up） | 点「开始指导 / 开始录制」 | 右上角 **✕** / 点背景 / 下拉 | 均**取消返回选择模式**，不启动 |
| 课前须知 → **拍摄技巧** | 「查看拍摄技巧」 | 左上 **‹ 返回** | 整页二级阅读 |
| 课前须知 → **数据保存** | 数据行「更改 / 开启」 | 点背景 / 「完成 / 这次不用」 | 二级小弹窗 |
...
- **训练数据保存（底部小行 + 二次弹窗）**：
  ...
  - **隐私提示要「恰到好处」（重要）**：**不做**强承诺（如「我们不会用你的视频训练 AI」——话说太满反而引发顾虑，且不想现在把话讲死）；**也不暗示**会拿视频改进算法（原「开启可帮助 ATOM 持续优化」已删）。弹窗内**只保留一个轻量入口**：「详见 · 隐私协议」，具体以隐私政策为准。开启邀请文案精简为「为本次开启即可获得复盘」。
  - **仅存 ATOM**（标注 **需 SD 卡**）：视频存 ATOM 的 SD 卡，可自行拷走，不上传云端。
  - **SD 卡状态 App 端不可知（重要）**：App **拿不到 ATOM 的 SD 卡状态**（未同步）→ **不能**做「已检测到 / 未检测到 SD 卡」这类确定态判断，也**无法保证**选了「仅存 ATOM」就一定存成功。因此「仅存 ATOM」只作**提示**：选中时给一条 info 提醒「App 端无法确认 SD 卡状态——请自行确保已插卡，否则本次不会保存」，**不做**「未检测到 SD 卡」告警。云端为可靠默认。
- **「不保存视频」= 全局设置，而非每次的选项（重要）**：
  ...
  - 逻辑**反向**：
    ...
    - 全局**关闭**：课前须知数据行变为**引导保存**——显示「视频保存已关闭，本次不会保存」+「**开启**」；点开是**「保存这次的视频？」**弹窗。
  - **开启范围让用户选（仅本次 / 一直开启）**：弹窗底部有勾选「**以后一直开启保存（可在设置关闭）**」——
    - **不勾（默认）= 仅本次临时开启**，全局设置不变，下次训练仍会询问；
    - **勾选 = 一直开启**，等于把 **Settings 全局开关**改回开。
  - **Record & Recap 对保存是硬需求**：它的产出就是「保存的视频 + 复盘」，没保存 = 没复盘（自相矛盾）。所以**全局关闭时选 Record & Recap**：课前须知显示**「需开启保存」**说明框，底部 CTA 变为**「开启保存以继续」**（点击进上面的开启弹窗），**未开启不能开始**。Live Coach（实时,不依赖留存）与 Manual Log（无摄像头）**不受影响**。
