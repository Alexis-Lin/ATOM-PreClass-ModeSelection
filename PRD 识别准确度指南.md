...
**✅ 这样摆**
- 全身入框、**站在画面中央**，四周留余量
- **ATOM 约膝盖高度最好**（推荐用配套 ATOM 三脚架最省事；放地面或你顺手的位置也行），别过度仰角对着脸
- 离 ATOM **0.5–1 米**
- **角度**：正对、侧对都 OK，**都不影响识别**；每个动作会有个让课中画面更「清爽」的角度，跟着课中提示摆即可，不用纠结。拉远/撤退的机位通常也没问题。
...
- **居中**：站在画面中间，不要偏到一侧。
- **距离**：**0.5–1 米**。ATOM 是贴地广角设备，靠得较近也能收全身；太近会裁切、太远会让关节点变小、识别变糊。
...
- **高度**：ATOM 贴地摆放，镜头约在 **膝盖高度**（不是胸高——它本就是放在地面/低处的设备）。
- **别过度仰角**：设备低、离得近会形成较大仰角；站够 ~0.7 米、让全身完整入框，避免极端仰拍把身体拉变形。
- **别倾斜**：设备放平，画面地平线要正。
- **正面或侧面**：正面最通用；部分动作需要侧面（如硬拉看背角），按课程提示摆放，但避免极端斜角。

---

## 3. 稳定摆放 · Stability

- 推荐用**配套的 ATOM 三脚架/支架**，把设备平稳架在**约膝盖高度**，画面更稳更平（也免去找平面）。
- 或放在**稳固的地面/低处平台**，别放在会滑动或晃动的东西上。
- 训练时地面震动可能让设备移位；确保它稳固、不被踢到。
- 一旦摆好，**开练后不要再移动**设备。
...
- **「正确 vs 错误」对照插画**（并排）：✓ 全身入框 / 稳定水平 / 顺光 vs ✕ 被裁切 / 仰拍 / 逆光 / 多人入镜。当前原型用的是示意火柴人，**正式版采用矢量插画**（非实拍照片），由设计师出一套更具代入感的矢量对照图。
