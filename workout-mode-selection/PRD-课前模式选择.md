...
- **训练数据保存（底部小行 + 二次弹窗）**：
  ...
  - **不保存视频**（标注 **最私密**）：不录制、不上传、不本地保存任何影像；**组数/次数/重量等训练数据仍会记入训练历史**（不保存的只是「视频」，不是「训练记录」）。隐私阶梯：云端 → 仅存 ATOM → 不保存。
    - 与模式相关：**Live Coach** 契合（纯实时，不留录像）；**Record & Recap** 的核心是「录制→复盘报告→日后再分析」，选「不保存」会**失去复盘报告**——当前设计在 Recap 里保留该选项但**明确提示**「不保存就没有复盘报告，想要纯实时可用 Live Coach」，且课前须知的「报告/留存」说明会相应变为该提示。（备选：Recap 里直接隐藏此选项，仅 Live Coach 提供。待定。）
  - 选择会被**记住**作为偏好，不必每次重选。
