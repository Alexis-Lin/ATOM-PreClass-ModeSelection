...
| **`Demo UX 素材图/`** | `UI-spec-en/zh.png`（全 20 屏总览）+ `flow-board.png`（交互流程图）+ `手机端/en·zh/`（iPhone 17 逐屏图）。 |
...
- **视频存储（只告知，不给选）**：智能模式强制上云，课前**没有存储选择**，只有一行只读告知「视频加密存到您的云端账户，练后随时回看复盘」+ 隐私协议。本地 SD 录制是 ATOM 端自己的开关，与 App 解耦。设置里「视频存储」本版为**只读状态**（默认开、无用户关闭入口）；全局隐私开关的接口已保留，未来放开后关闭会让两个智能模式置灰。
