...
- **具名三选一**：Live Coach / Record & Recap / Manual Log；未选中只显示标题，选中才展开说明。「课中随时能切」为卡片下方绿色小字，离线时不显示。
- **门槛**：两个 AI 模式需 **ATOM + Plus**；ATOM 离线拦截启动。Manual Log 永远可用。
- **课前须知（正向）**：教练心智「ATOM 就像教练的眼睛」+ 一张 OK 取景图 + 做到清单（**ATOM 约膝盖高度 · 离 0.5–1 米 · 居中 · 正/侧对 · 别被器械挡**）+「查看拍摄技巧」入口。反例/易错放到单独的**拍摄技巧页**。
...
品牌绿确切色值 · 「正确 vs 错误」取景对照插画 · 课程数据（时长/动作/消耗）· 配套三脚架型号 · 隐私协议链接。
